function new_timetable = updateTimetable_v5(timetable, regular, vgamma, blocksections, settings, trains, cancelled)
...
for tt = 1:Ntrains
    ...
    if direction(tt) == 12
		if vgamma(tt)
            [~,approachtime(tt,1)] = returnFirstApproachTime(blocksections, settings, traintype, direction(tt));
        else
            [approachtime(tt,1),~] = returnFirstApproachTime(blocksections, settings, traintype, direction(tt));
        end
        for bb = 2:Nblocks
            try
                if vgamma(tt)   % GAMMA
                    approachtime(tt,bb) = regTT.running(find(regTT.train_id == trains(tt)...
                                        & regTT.blocksection==bb-1));
                else
                    approachtime(tt,bb) = timetable.running(find(timetable.train_id == trains(tt)...
                                            & timetable.blocksection==bb-1));
                end
            end
            if (direction(tt) == settings.disruption.direction) && ~settings.disruption.signals && ~vgamma(tt)  % GAMMA
                try
    ...
    if direction(tt) == 13
        ...
        if vgamma(tt)
            [~,approachtime(tt,end)] = returnFirstApproachTime(blocksections, settings, traintype, direction(tt));
        else
            [approachtime(tt,end),~] = returnFirstApproachTime(blocksections, settings, traintype, direction(tt));
        end
...
%                 if timetable.adjusted_departure(rows_train(rr)) == 0
                    ...
                    if vgamma(tt)
                        timetable.adjusted_departure(rows_train(rr)) = ...
                            timetable.adjusted_arrival(rows_train(rr)) + regTT.running(rows_train(rr));
                    else
                         timetable.adjusted_departure(rows_train(rr)) = ...
...
%             try
%                 if timetable.adjusted_arrival(ee) == timetable.adjusted_departure(ee-1)
%                     % Happening as smooth movement => we have to include
%                     % the approachtime.
%                     timetable.adjusted_start(ee) = timetable.adjusted_arrival(ee) ...
%                                     - t_setup - approachtime(tt,bs);
%                 else
%                     % Train comes to a stop, no approach time needed for
%                     % the next one!
%                     timetable.adjusted_start(ee) = timetable.adjusted_arrival(ee) ...
%                                     - t_setup;
%                 end
...
if settings.disruption.signals == 0
    ...
    tt_id = setdiff(tt_id, trains(find(vgamma == 1)));
    
    for tt = 1:length(tt_id)
        if cancelled(find(trains == tt_id(tt))) == 0 && deviated(find(trains == tt_id(tt))) == 0
