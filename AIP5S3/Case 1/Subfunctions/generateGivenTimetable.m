function [base_tt, hour_tt, complete_tt] = generateGivenTimetable(settings,blocksections,runningtimes,rawdata)
...
rows = find(strcmp(traintype,'R') & direction == 1);
...
% R trains
rows = find(strcmp(traintype,'R') & direction == 0);
for rr = 1:length(rows)
    time = entrytime(rows(rr));
