function [base_tt, hour_tt, complete_tt] = generateTimetable_v2(settings,blocksections,runningtimes,clearingtimes);
...
time = offsetdirections + settings.TT.delta.dir1*60;  
...
time =  settings.TT.delta.dir0*60;
...
for ee = 1:size(timetable,2)
    ...
    switch timetable(ee).train_type
        case 'IC'
            ...
            if timetable(ee).direction == 1
                t_release_IC = clearingtimes.IC1.regular(block);
            else
                t_release_IC = clearingtimes.IC0.regular(block);
            end
            timetable(ee).finish = timetable(ee).departure + t_release_IC;
        case 'R'
            block = timetable(ee).blocksection;
            if timetable(ee).direction == 1
                t_release_R = clearingtimes.L1.regular(block);
            else
                t_release_R = clearingtimes.L0.regular(block);
            end
