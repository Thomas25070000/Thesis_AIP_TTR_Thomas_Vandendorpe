function [timetable, solu, measures, statistics] = modelCase1_singleMachine_full(timetable, regular, blocksections, trains, minHW, settings, runningtimes)
try
        ...
        if jj>ii
            ...
            if direction(ii) ~= direction(jj)
                if direction(ii) == settings.disruption.direction
                    seq1 = [t(jj,closedM) + bigM * (1 - q(ii,jj) + gamma(ii) + x(ii) + dev(ii)) ...
                            - ref_event_ii - t_setup_ii_jj >= 0]: label;
                    seq2 = [t(ii,closedM) + bigM * (q(ii,jj) + gamma(ii) + x(jj) + dev(jj)) ...
                            - ref_event_jj - t_setup_jj_ii >= 0]: label;    
                else
                    seq1 = [t(jj,closedM) + bigM * (1 - q(ii,jj) + gamma(jj) + x(ii) + dev(ii)) ...
                            - ref_event_ii - t_setup_ii_jj  >= 0]: label;
                    seq2 = [t(ii,closedM) + bigM * (q(ii,jj) + gamma(jj) + x(jj) + dev(jj)) ...
                            - ref_event_jj - t_setup_jj_ii  >= 0]: label;
                end
            else
                seq1 = [t(jj,closedM) + bigM * (1 - q(ii,jj) + gamma(ii) + x(ii) + dev(ii)) ...
                ...
                if direction(ii) == 1
                    ...
                    if releasetime(jj) > releasetime(ii)
                        ...
                        for mm = 1:Nmachines-1
                            if bref(mm) > 0
                                ...
                                seqConstraint = [t(jj,mm) - t(ii,mm+1) - t_setup ...
                                        + bigM * (x(ii)+x(jj)+dev(ii)+dev(jj)) - delta_approach * gamma(jj) >= 0];   % GAMMA
						...
						if closedM ~= Nmachines
							seqConstraint = [t(jj,end) - C(ii) - t_setup + bigM * (x(ii)+x(jj)+dev(ii)+dev(jj))...
                        ...
                        for mm = 1:Nmachines-1
                            if bref(mm) > 0         % Don't do it for the disrupted section!
                                ...
                                seqConstraint = [t(ii,mm) - t(jj,mm+1) - t_setup + bigM * (x(ii)+x(jj)+dev(ii)+dev(jj)) ...
						...
						if closedM ~= Nmachines
							seqConstraint = [t(ii,end) - C(jj) - t_setup + bigM * (x(ii)+x(jj)+dev(ii)+dev(jj)) ...
											- delta_approach * gamma(ii)>= 0];
							corridorSeq = [corridorSeq seqConstraint];
						end
                    end
...
%                         t_setup = cleartime(ii) + t_before + processtime(jj,end);
                        for mm = Nmachines:-1:2
                            if bref(mm)>0
                                ...
                                seqConstraint = [t(jj,mm) - t(ii,mm-1) - t_setup + bigM * (x(ii)+x(jj)+dev(ii)+dev(jj))...
                                             - delta_approach * gamma(jj)>= 0];
                                corridorSeq = [corridorSeq seqConstraint];
                            end
                        end
						...
						if closedM ~= 1
							seqConstraint = [t(jj,1) - C(ii) - t_setup + bigM * (x(ii)+x(jj)+dev(ii)+dev(jj)) ...
										-  delta_approach * gamma(jj)>= 0];
							corridorSeq = [corridorSeq seqConstraint];
						end
                    else
%                         t_setup = cleartime(jj) + t_before + processtime(ii,end);
						...
						if closedM ~= 1
							seqConstraint = [t(ii,1) - C(jj) - t_setup + bigM * (x(ii)+x(jj)+dev(ii)+dev(jj)) ...
											- delta_approach * gamma(ii) >= 0];
							corridorSeq = [corridorSeq seqConstraint];
						end
                    end
