...
%     end
    ...
    if strcmp(traintype(tt),'THA')
        maxDelay = [d(tt) <= maxD_THA]: ['maxDelay_' int2str(trains(tt))];
    else
    ...
    for jj = ii:Ntrains
        if jj>ii
            ...
            if direction(ii) ~= direction(jj)
                t_setup_ii_jj = cleartime + approach_other(jj) + t_before;
                t_setup_jj_ii = cleartime + approach_other(ii) + t_before;
            else
                t_setup_ii_jj = cleartime + approachtime(jj,closedM) + t_before;
                t_setup_jj_ii = cleartime + approachtime(ii,closedM) + t_before;
            end
            ...
            ref_event_ii = t(ii,closedM) + proctimes.disrupted(ii,closedM);
...
% 						t_setup_ii_jj = cleartime + approachtime(jj,end) + t_before;
% 						t_setup_jj_ii = cleartime + approachtime(ii,end) + t_before;
