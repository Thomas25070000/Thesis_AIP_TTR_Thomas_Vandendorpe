...
for ii = 1:Ntrains
    for jj = ii:Ntrains
        if jj>ii
            if (direction(ii)==3 && direction (jj)==13)||(direction(ii)==13 && direction(jj)==3)
                test = 1;
            else
                label = ['seq_' int2str(ii) '_' int2str(jj)];
                seq1 = [t(jj) + bigM * (1 + x(ii)+x(jj) - q(ii,jj)) - t(ii) ...
                        - setuptime(ii,jj) >= 0]: label;
                seq2 = [t(ii) + bigM * (x(ii)+x(jj)+q(ii,jj)) - t(jj) ...
...
%     Objective = Objective + C(tt) - deptime(tt);
    if direction(tt) == 13 || direction(tt) == 3
...
for tt = 1:Ntrains
    if direction(tt) == 13 || direction(tt) == 3
        Objective = Objective + 10000*w_cancel * x(tt);
    else
