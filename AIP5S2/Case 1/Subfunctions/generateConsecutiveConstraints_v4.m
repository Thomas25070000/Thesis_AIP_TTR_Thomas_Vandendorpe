function consecutiveConstraints = generateConsecutiveConstraints_v4(timetable, trains, settings, q, x, dev, gamma)
...
%         C = [Q2 - Q1 - limit * cancel_factor - length(second_comp) * (x(first) + x(second) + gamma(second)) <= 0]: label;
    ...
    label = ['nr_trains_before_' int2str(trains(first))];
