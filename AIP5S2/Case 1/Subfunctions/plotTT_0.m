function [line, blocks, YTick, YTick_HHMMSS] = plotTT_0(timetable, blocksections, settings, type, include_blocks, firstTime)
...
timetable(timetable.direction == 12,:) = [];
timetable(timetable.direction == 13,:) = [];
...
ylabel('Time [H:MM]');
xlabel('Distance [m]');
title('Regular timetable during busiest hour on section A, lower track');
