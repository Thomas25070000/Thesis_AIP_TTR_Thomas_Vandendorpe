function [line, blocks] = plotTT_1(timetable, blocksections, settings, type, include_blocks, firstTime, YTick, YTick_HHMMSS)
...
timetable(timetable.direction == 2,:) = [];
timetable(timetable.direction == 3,:) = [];
...
createTitle = [settings.general.caseName ' ' settings.general.subName ...
                    ' original'];
title('Regular timetable during busiest hour on section A, upper track');
