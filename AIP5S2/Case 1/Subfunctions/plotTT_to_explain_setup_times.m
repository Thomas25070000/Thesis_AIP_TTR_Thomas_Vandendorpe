...
% timetable.running(1:6) = timetable.running(1:6)/4;
% for index_here=1:6
...
% end
% timetable.start(1) = timetable.departure(1)-2*timetable.running(1);
% 
headway = timetable.finish(7)-timetable.start(6)
timetable = delayTrain(timetable, 113, headway, 1);
%timetable.start(10) = timetable.start(10)-30;
%timetable.start(11) = timetable.start(11)-30;
line = [];
ll = 0;
blocks = [];
bb = 0;
