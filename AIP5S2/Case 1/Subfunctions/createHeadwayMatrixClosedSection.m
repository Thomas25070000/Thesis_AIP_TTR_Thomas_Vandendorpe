...
for ii = 1:length(trains)-1
    for jj = (ii+1):length(trains)
        if abs(direction(ii)-direction(jj))==0
...
%             minHW(jj,ii) = max(min_HW_jj_ii, norm_jj_ii)+release_ii_jj;
            ...
            minHW(ii,jj) = min_HW_ii_jj+release_jj_ii;
            minHW(jj,ii) = min_HW_jj_ii+release_ii_jj;
        end
        if (abs(direction(ii)-direction(jj))==1) && (direction(ii)~=disruption_dir && direction(jj)~=disruption_dir)
            % Trains are running in the same direction. Regardless of the
            % train type (they are running at the same speed), we get the
            % same headway!).
...
%             minHW(jj,ii) = max(min_HW_jj_ii, norm_jj_ii)+release_ii_jj;
        ...
        if (abs(direction(ii)-direction(jj))==1) && (direction(ii)==disruption_dir || direction(jj)==disruption_dir)
            rows_ii = find(timetable.train_id == trains(ii)& ismember(timetable.blocksection, blocksections.id(blocksections.closed==1)));
            rows_jj = find(timetable.train_id == trains(jj)& ismember(timetable.blocksection, blocksections.id(blocksections.closed==1)));            
            if timetable.start(rows_jj(end))-timetable.finish(rows_ii(end)) < crossing_threshold
                extra_setup = crossing_threshold - (timetable.start(rows_jj(end))-timetable.finish(rows_ii(end)));
                min_HW_ii_jj = extra_setup;
            end
            if timetable.start(rows_ii(end))-timetable.finish(rows_jj(end)) < crossing_threshold
                extra_setup = crossing_threshold - (timetable.start(rows_ii(end))-timetable.finish(rows_jj(end)));
                min_HW_jj_ii = extra_setup;
            end
            % If norms are to be used!
            % Timetable only has those running over the closed sections.
%             norm_ii_jj = settings.TT.headway.same * 60;
...
%             minHW(ii,jj) = max(min_HW_ii_jj, norm_ii_jj)+release_jj_ii;
%             minHW(jj,ii) = max(min_HW_jj_ii, norm_jj_ii)+release_ii_jj;
            minHW(ii,jj) = min_HW_ii_jj;
            minHW(jj,ii) = min_HW_jj_ii;
%             minHW(ii,jj) = min_HW_ii_jj+release_jj_ii;
%             minHW(jj,ii) = min_HW_jj_ii+release_ii_jj;
        end
        default_from_sander = 12 + 9;
        if (abs(direction(ii)-direction(jj))==10) && (direction(ii)==disruption_dir || direction(jj)==disruption_dir) 
            if direction(ii)>10
                rows_ii = find(timetable.train_id == trains(ii));
...
%                 minHW(jj,ii) = -10000;
        ...
        if (abs(direction(ii)-direction(jj))==10) && (direction(ii)~=disruption_dir && direction(jj)~=disruption_dir) 
            %might have to delete the constraints here
            minHW(ii,jj) = -10000;
            minHW(jj,ii) = -10000;
        end
        if abs(direction(ii)-direction(jj))==9
            rows_ii = find(timetable.train_id == trains(ii)& ismember(timetable.blocksection, blocksections.id(blocksections.closed==1)));
        ...
        if abs(direction(ii)-direction(jj))==11
            rows_ii = find(timetable.train_id == trains(ii)& ismember(timetable.blocksection, blocksections.id(blocksections.closed==1)));
            rows_jj = find(timetable.train_id == trains(jj)& ismember(timetable.blocksection, blocksections.id(blocksections.closed==1)));
            t_closed_ii = sum(timetable.running(rows_ii));
            t_closed_jj = sum(timetable.running(rows_jj));
            if direction(ii) == 2
                extra_setup = crossing_threshold - (timetable.start(rows_jj(end))-timetable.finish(rows_ii(1)));
                min_HW_ii_jj = extra_setup;
%                     min_HW_ii_jj = extra_setup+t_closed_ii;

                extra_setup = crossing_threshold - (timetable.start(rows_ii(1))-timetable.finish(rows_jj(end)));
                %min_HW_jj_ii = extra_setup;
                min_HW_jj_ii = extra_setup+t_closed_jj;

            end
            if direction(ii) == 13
                extra_setup = crossing_threshold - (timetable.start(rows_jj(1))-timetable.finish(rows_ii(end)));
                %min_HW_ii_jj = extra_setup;
                min_HW_ii_jj = extra_setup+t_closed_ii;
                extra_setup = crossing_threshold - (timetable.start(rows_ii(end))-timetable.finish(rows_jj(1)));
                min_HW_jj_ii = extra_setup;
%                     min_HW_jj_ii = extra_setup+t_closed_jj;
            end
%             if direction(ii) == 2
%                 if timetable.start(rows_jj(end))-timetable.finish(rows_ii(1)) < crossing_threshold
%                     extra_setup = crossing_threshold - (timetable.start(rows_jj(end))-timetable.finish(rows_ii(1)));
%                     min_HW_ii_jj = extra_setup;
% %                     min_HW_ii_jj = extra_setup+t_closed_ii;
%                 end
%                 if timetable.start(rows_ii(1))-timetable.finish(rows_jj(end)) < crossing_threshold
%                     extra_setup = crossing_threshold - (timetable.start(rows_ii(1))-timetable.finish(rows_jj(end)));
%                     %min_HW_jj_ii = extra_setup;
%                     min_HW_jj_ii = extra_setup+t_closed_jj;
%                 end
%             end
%             if direction(ii) == 13
%                 if timetable.start(rows_jj(1))-timetable.finish(rows_ii(end)) < crossing_threshold
%                     extra_setup = crossing_threshold - (timetable.start(rows_jj(1))-timetable.finish(rows_ii(end)));
%                     %min_HW_ii_jj = extra_setup;
%                     min_HW_ii_jj = extra_setup+t_closed_ii;
%                 end
%                 if timetable.start(rows_ii(end))-timetable.finish(rows_jj(1)) < crossing_threshold
%                     extra_setup = crossing_threshold - (timetable.start(rows_ii(end))-timetable.finish(rows_jj(1)));
%                     min_HW_jj_ii = extra_setup;
% %                     min_HW_jj_ii = extra_setup+t_closed_jj;
%                 end
