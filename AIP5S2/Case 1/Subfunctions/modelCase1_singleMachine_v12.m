function [timetable, solu, measures, statistics] = modelCase1_singleMachine_v12(timetable, regular, blocksections, trains, minHW, settings, runningtimes)
try
            ...
            if direction(ii) == direction(jj)
                ...
                if direction(ii) == 1
                    ...
                    if releasetime(jj) > releasetime(ii)
                        ...
                        for mm = 1:Nmachines-1
                            if bref(mm) > 0
                                ...
                                seqConstraint = [t(jj,mm) - t(ii,mm+1) - t_setup ...
                                        + bigM * (x(ii)+x(jj)) - delta_approach * gamma(jj) >= 0];   % GAMMA
                        ...
                        minHW_right = right_end_HW - processtime(jj,end);
                        if minHW_right >= t_setup
                            t_setup = minHW_right;
                            delta_approach = regprocesstime(jj,end) - processtime(jj,end);
                        end
						if closedM ~= Nmachines
							seqConstraint = [t(jj,end) - C(ii) - t_setup + bigM * (x(ii)+x(jj))...
                        ...
                        for mm = 1:Nmachines-1
                            if bref(mm) > 0         % Don't do it for the disrupted section!
                                ...
                                seqConstraint = [t(ii,mm) - t(jj,mm+1) - t_setup + bigM * (x(ii)+x(jj)) ...
                        ...
                        minHW_right = right_end_HW - processtime(ii,end);
                        if minHW_right >= t_setup
                            t_setup = minHW_right;
                            delta_approach = regprocesstime(ii,end) - processtime(ii,end);
                        end
						if closedM ~= Nmachines
							seqConstraint = [t(ii,end) - C(jj) - t_setup + bigM * (x(ii)+x(jj)) ...
											- delta_approach * gamma(ii)>= 0];
							corridorSeq = [corridorSeq seqConstraint];
						end
                    end
...
%                         t_setup = cleartime(ii) + t_before + processtime(jj,end);
                        for mm = Nmachines:-1:2
                            if bref(mm)>0
                                ...
                                seqConstraint = [t(jj,mm) - t(ii,mm-1) - t_setup + bigM * (x(ii)+x(jj))...
                                             - delta_approach * gamma(jj)>= 0];
                                corridorSeq = [corridorSeq seqConstraint];
                            end
                        end
                        ...
                        minHW_left = left_end_HW - processtime(jj,1);
                        if minHW_left >= t_setup
                            t_setup = minHW_left;
                            delta_approach = regprocesstime(jj,1) - processtime(jj,1);
                        end
                        
						if closedM ~= 1
							seqConstraint = [t(jj,1) - C(ii) - t_setup + bigM * (x(ii)+x(jj)) ...
										-  delta_approach * gamma(jj)>= 0];
							corridorSeq = [corridorSeq seqConstraint];
						end
                    else
%                         t_setup = cleartime(jj) + t_before + processtime(ii,end);
                        ...
                        minHW_left = left_end_HW - processtime(ii,1);
                        if minHW_left >= t_setup
                            t_setup = minHW_left;
                            delta_approach = regprocesstime(ii,1) - processtime(ii,1);
                        end
						
						if closedM ~= 1
							seqConstraint = [t(ii,1) - C(jj) - t_setup + bigM * (x(ii)+x(jj)) ...
											- delta_approach * gamma(ii) >= 0];
							corridorSeq = [corridorSeq seqConstraint];
						end
                    end
...
% Due to the number of passengers being known, we get new waits for all
% trains!
ratio = pax / sum(pax) * Ntrains;

% Penalize the delays
for tt = 1:Ntrains
%     Objective = Objective + C(tt) - deptime(tt);
    if mod(trains(tt),10) == 1
        Objective = Objective + w_delay * w_dir1 * ratio(tt) * d(tt);
        Objective = Objective + w_dir1 * w_late_entry * ratio(tt) * e(tt);     % Very small penalty for an entrance delay.
    else
        Objective = Objective + w_dir0 * w_delay * ratio(tt) * d(tt);
        Objective = Objective + w_dir0 * w_late_entry * ratio(tt) * e(tt);
    end
end
...
for tt = 1:Ntrains
    if mod(trains(tt),10) == 1
        Objective = Objective + w_cancel * w_dir1 * ratio(tt) * x(tt);
    else
        Objective = Objective + w_cancel * w_dir0 * ratio(tt) * x(tt);
    end
end
