function [base_tt, hour_tt, complete_tt] = generateTimetable(settings,blocksections)
...
for bb = 1:size(blocksections,1)
    ...
    time = time + round((blocksections.length(bb) / speedIC) * buffer_factor);
...
for bb = 1:size(blocksections,1)
    ...
    timetable(tt).blocksection = bb;
    timetable(tt).arrival = time;
    time = time + round((blocksections.length(bb) / speedR) * buffer_factor);
    timetable(tt).departure = time;
    timetable(tt).running = timetable(tt).departure - timetable(tt).arrival;
end
